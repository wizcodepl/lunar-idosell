...
- Laravel version
- PHP version
- Idosell plan / API version
- A minimal reproduction (the smallest payload or test that triggers the bug)
- Expected vs. actual behavior

If you suspect a security issue (e.g. token leakage, leaked secrets in logs), do **not** open a public issue — email security@wizcode.pl.
...
4. Run the formatter and static analysis:
   ...
   composer analyse
   ```
5. Open a PR against `main` with a description of the change and any context (linked issue, Idosell docs reference, etc.).
